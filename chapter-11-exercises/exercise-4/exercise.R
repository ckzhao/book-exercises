...
gained <- mutate(flights, delayed_in_air = arr_delay - dep_delay)
View(flights)
...
summarize(to_sea, avg_delayed = mean(delayed_in_air, narm = TRUE))

# Consider flights from JFK to SEA. What was the average, min, and max air time
# of those flights? Bonus: use pipes to answer this question in one statement
# (without showing any other data)!
flights %>% 
  filter(flights, max(air_time), min(air_time), mean(air_time)) %>% 
