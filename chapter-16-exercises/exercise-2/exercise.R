...
ggplot(data = diamonds_sample) +
  geom_col(mapping= aes(x = price, y = clarity)) 

# Draw the same chart again, but with each element positioned to "fill" the y axis
  ggplot(data = diamonds_sample) +
    geom_col(mapping= aes(x = price, y = fill)) 

# Draw the same chart again, but with each element positioned to "dodge" each other
  ggplot(data = diamonds_sample) +
    geom_col(mapping= aes(x = price, y = dodge)) 
...
# previous exercise).
  ggplot(data = diamonds_sample) +
    geom_col(mapping= aes(x = price, y = fill), position = jitter) 
...
ggplot(data = diamonds_sample) +
  geom_col(mapping= aes(x = cut, y = cut), width = 1) 
  

# Draw the same chart, but with the coordinate system flipped
ggplot(data = diamonds_sample) +
  geom_col(mapping= aes(x = cut, y = cut), width = 1, direction = -1) 
